...
async function loadCMSGallery(): Promise<{ src: string; alt: string }[] | null> {
  try {
...
export default async function GalleryPage() {
  const cmsImages = await loadCMSGallery();
  const combined = cmsImages ?? [];
  const seen = new Set<string>();
  const source = combined.filter((img) => {
    if (seen.has(img.src)) return false;
    seen.add(img.src);
    return true;
  });
  const first = source.slice(0, 10);
  const rest = source.slice(10);
